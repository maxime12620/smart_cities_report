...
nf(1) = 0; 
nth = length(TCd{1}{5});    % no of temp nodes (f) of TCd1 
tcth = ones(1,nth);         % index for nodes of thermal circ.
nq = length(TCd{1}{3});     % no of flow branches (b) of TCd1
tcq = ones(1,nq);           % index for branches of thermal circ.
gq(1) = length(TCd{1}{3});  % global flow: start with TCd1
gth(1) = length(TCd{1}{5}); % global temp: start with TCd1
for n = 2:size(TCd,2)
    nf(n) = nf(n-1) + length(TCd{n-1}{5});  % total # flows
